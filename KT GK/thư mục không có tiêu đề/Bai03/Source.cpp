...
int Bai03(vector<int> arr, int n)
{
    ...
    vector<int> sumArri_index;   // Tao mang chua Ai + |i^1.5|
    vector<int> minusArrj_index; // Tao mang chua Aj - |canj|
    ...
    // Tim max, min cua mang chua Aj - |canj|
    int max = minusArrj_index[0];
    int min = minusArrj_index[0];
    for (int i = 0; i < n; i++)
    {
        if (minusArrj_index[i] > max)
            max = minusArrj_index[i];
        else if (minusArrj_index[i] < min)
            min = minusArrj_index[i];
    }

    // Tao mang moi chua so lan xuat hien cua cac phan tu trong mang chua Aj - |canj|
    int k = max - min + 1;
    ...
    // Dua vao mang Count de dem so lan xuat hien cua cac phan tu chua Ai + |i^1.5|
    int ans = 0;
    for (int i = 0; i < n; i++)
    {
