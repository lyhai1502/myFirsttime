...
void function01(List &L)
{
    Node *left = L.p_head;
    Node *right = L.p_tail;

    while (left != right && right->p_next != left)
    {
        right->p_next = left->p_next;
        left->p_next = right;

        left = left->p_next;
        right = pre_Node(L, right);
    }
    left->p_next = NULL;
}
int main()
{
    ...
    // cout << pre_Node(L, p)->key;

    if (L.p_tail == NULL)
        cout << "co null"; //function01(L, n);
    //printList(L);
    return 0;
}