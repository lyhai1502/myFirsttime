...
List reverseFromLeftToRight(List L, int left, int right)
{
    Node *doDai = L.p_head;
    Node *p_prevleft;
    Node *p_right;
    ...
    List revList;
    if (left == 0)
        revList.p_head = L.p_head;
    else
        revList.p_head = p_prevleft->p_next;
    revList.p_tail = p_right;
    Node *p_nextright = p_right->p_next;
    if (right < index)
        p_right->p_next = NULL;
    revList = reverseLinked_List(revList);
    if (left == 0)
        L.p_head = revList.p_head;
    else
        p_prevleft->p_next = revList.p_head;
    if (right == index)
        revList.p_tail->p_next = NULL;
    else
        revList.p_tail->p_next = p_nextright;
    return L;
}
