...
NODE *rightRotate(NODE *A)
{
    ...
    // Cap nhat chieu cao
    A->height = findMax(height(A->left), A - height(A->right)) + 1;
    X->height = findMax(height(X->left), X - height(A->right)) + 1;
...
NODE *leftRotate(NODE *B)
{
    ...
    X->height = findMax(height(X->left), height(B->right)) + 1;
...
NODE *updateTree(NODE *root, int x)
{
