...
void removeDuplicateElements_dList(dList &L)
{
    int count = 1;
    int temp = 1;
    for (dNode *p = L.p_head; p->p_next != NULL; p = p->p_next)
    {
        for (dNode *q = p->p_next; q != NULL; q = q->p_next)
        {
            count++;
            if (p->key == q->key)
            {
                dNode *q_left = q->p_prev;
                dNode *q_right = q->p_next;
                removeElementIndex_dList(L, count);
                q = q_left;
                q_left->p_next = q_right;
                cout << q->key << endl;
            }
            count = ++temp;
        }
...
int main()
{
    ...
    addElementIndex_dList(ls, 4, 13);
    addElement_dList(ls, 66);
    addElement_dList(ls, 77);
    print_dList(ls);
    cout << endl;
    removeDuplicateElements_dList(ls);
    print_dList(ls);
    return 0;
}