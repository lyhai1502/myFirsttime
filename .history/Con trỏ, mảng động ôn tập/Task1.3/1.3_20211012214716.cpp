...
Cai dat ham doc thong tin danh sach cac thi sinh void docDanhSachThiSinh(const char *tenFile)
{
    ifstream ifs;
    ifs.open(tenFile, ios::in);
    if (!ifs.is_open())
    ...
    while (!ifs.eof())
    {
        str = new char[255];
        ifs.getline(str, 255);
        list.push_back(docThongTinThiSinh(str));
    }
}
...
int main()
{
    char *s = "BD1200001,a van nguyen,4.0,5.0,,,,4.25,7.0,7.75,,,2.0,N1,BinhDinh";
    thiSinh x = tachChuoiTuFile(s);
    cout << x.lichSu;
    return 0;
}
