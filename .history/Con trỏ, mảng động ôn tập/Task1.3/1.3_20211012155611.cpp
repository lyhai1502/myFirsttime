...
char **tachChuoi(string line, string delimiter)
{
    ...
    while ((pos = line.find(delimiter)) != std::string::npos)
    {
        ...
        if (token == "")
            token = "0";
        newChar[indexArray++] = token;
        line.erase(0, pos + delimiter.length());
    }
    newChar[indexArray] = line;
    return newChar;
}
...
int main()
{
    string line = "BD1200001,a van nguyen,4.0,5.0,,,,4.25,7.0,7.75,,,2.0,N1,BinhDinh";
    string deli = ",";
    char **newChar = tachChuoi(line, deli);
    cout << newChar[1];
