...
char *trungToSangHauTo(char *trungTo)
{
    ...
    for (indexTrungTo = 0; trungTo[indexTrungTo] != '\0'; indexTrungTo++)
    {
        ...
        else
        {
            while ((doUuTien(token) <= doUuTien((char)(toanTu.head->value + '0'))) && !isStack_Empty(toanTu))
                hauTo[indexHauTo++] = (char)(popStack(toanTu) + '0');
...
// char *trungToSangTienTo(char *trungTo)
// {
//     char *prefix = new char;
//     stackNode operators = createStack();
//     char token;
//     int indexInfix = 0, indexPrefix = 0;
//     for ()
// }