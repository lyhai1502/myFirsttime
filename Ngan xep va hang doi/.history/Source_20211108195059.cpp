...
char *trungToSangHauTo(char *trungTo)
{
    ...
    while (trungTo[indexTrungTo] != 'm')
    {
        token = trungTo[indexTrungTo++];
        if ((int)token - 48 >= 0 && (int)token - 48 <= 9)
            hauTo[indexHauTo++] = token;
        else
        {
            while (doUuTien(token) <= doUuTien((char)(toanTu.head->value + 48)) && !isStack_Empty(toanTu))
            {
                x = (char)(popStack(toanTu) + 48);
                hauTo[indexHauTo++] = x;
            }
            pushStack(toanTu, (int)token - 48);
        }
    }
