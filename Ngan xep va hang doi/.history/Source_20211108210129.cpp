...
char *infixToPostfix(char *infix)
{
    ...
    for (indexInfix = 0; infix[indexInfix] != '\0'; indexInfix++)
    {
        ...
        // else
        // {
        //     while ((doUuTien(token) <= doUuTien((char)(allOperators.head->value + '0'))) && !isStack_Empty(allOperators))
        //         postfix[indexPostfix++] = (char)(popStack(allOperators) + '0');
        else
        {
            while (!isStack_Empty(allOperators) && (doUuTien(token) <= doUuTien((char)(allOperators.head->value + '0'))))
                postfix[indexPostfix++] = (char)(popStack(allOperators) + '0');
            pushStack(allOperators, (int)token - '0');
            // }
        }
        cout << endl;
        printStack(allOperators);
        cout << endl;
    }
