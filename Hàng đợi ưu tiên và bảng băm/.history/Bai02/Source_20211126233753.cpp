...
void readHashTable(CongTy *hashTable, const char *tenFile)
{
    ...
    while (!ifs.eof())
    {
        ...
        //cout << HashString(ct.tenCongTy) << " " << hashTable[HashString(ct.tenCongTy)].diaChi << endl;
    }
    ifs.close();
}
int main()
{
    string s = "CONG TY CO PHAN XAY DUNG DAU TU PHAT TRIEN DI SAN SAO VIET";
    readHashTable(hashTable, "mst.txt");
    cout << hashTable[HashString(s)].maSoThue << endl;
    return 0;
}